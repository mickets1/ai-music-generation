...
def convert_audio_image(image_path: str, crop_width: int = None) -> AudioSegment:
...
def process_and_concatenate_audios(input_folder: str, output_file: str):
    audio_segments = []
    for filename in sorted(os.listdir(input_folder)):
        if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp')):
            image_path = os.path.join(input_folder, filename)
            audio_segment = convert_audio_image(image_path)
...
if __name__ == "__main__":
    ...
    OUTPUT_FILE = os.path.join("OUTPUT_FOLDER", "joined_audio.mp3")
    process_and_concatenate_audios(INPUT_FOLDER, OUTPUT_FILE)
